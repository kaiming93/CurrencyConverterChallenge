...
const CurrencyConverter = () => {
  ...
  const [previousCountry, setPreviousCountry] = React.useState<
    Array<JSX.Element | any>
  >([
    { key: "GBP", name: "GBP/British Pound Sterling" },
    { key: "EUR", name: "EUR/Euro" },
  ]);
  const [country, setCountry] = React.useState<Array<JSX.Element | any>>([
    { key: "GBP", name: "GBP/British Pound Sterling" },
    { key: "EUR", name: "EUR/Euro" },
  ]);
  const [selectState, setSelectState] = React.useState<Array<boolean>>([
    false,
    false,
  ]);
  ...
  const calculateResult = (event: any) => {
    ...
    if (!amount || isNaN(amount) || !reg.test(String(amount))) {
      ...
      let result =
        (Number(amount) * Number(rates[country[1].key])) /
        Number(rates[country[0].key]);
      setCounter(600);
  ...
  return (
    <div data-testid="currency-converter" className="currency-converter">
      ...
      />
      {rates && (
        <div className="rates" data-testid="rates">
          <p>
            {Number(rates[country[0].key]) / Number(rates[country[0].key])}{" "}
            {country[0].key} ={" "}
            {(
              Number(rates[country[1].key]) / Number(rates[country[0].key])
            ).toFixed(10)}{" "}
            {country[1].key}
          </p>
        </div>
